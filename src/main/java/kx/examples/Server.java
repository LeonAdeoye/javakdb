...
public class Server{
  ...
  private static class ServerConnection extends Connection implements AutoCloseable{
    ServerConnection(ServerSocket s)throws java.io.IOException{super(s);}
    @Override
  ...
  public static void main(String[] args){// example echo server for a single client
    int port=5010;
    try (ServerConnection c = new ServerConnection(new ServerSocket(port))) {
      boolean ok = true;
      while(ok){

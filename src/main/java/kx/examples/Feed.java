...
public class Feed{
   ...
   */
  static void rowInserts(Connection kconn) throws java.io.IOException{
    ...
    for(int i=0;i<10;i++){
      ...
      Object[] row={new Connection.Timespan(),"SYMBOL",Double.valueOf(93.5),Long.valueOf(300L)};
   ...
   * @param kconn Connection that will be sent the inserts
   * @throws java.io.IOException when issue with KDB+ comms
   * @throws Connection.KException if request evaluation resulted in an error
   */
  static void bulkInserts(Connection kconn) throws java.io.IOException, Connection.KException{
    ...
    Connection.Timespan[] time=new Connection.Timespan[10];
    ...
    for(int i=0;i<10;i++){
      time[i]=new Connection.Timespan();
    ...
    kconn.ks(QFUNC,TABLENAME,new Connection.Flip(new Connection.Dict(new String[]{"time","sym","price","size"},new Object[]{time,sym,price,size})));
  ...
  public static void main(String[] args){// example tick feed
    ...
    try{
      ...
      rowInserts(Connection);
      bulkInserts(Connection);
    }
    catch(Exception e){

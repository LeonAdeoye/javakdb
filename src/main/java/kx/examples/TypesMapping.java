  ...
  public static void main(String[] args){
    ...
    try{
      ...
      Object[]vectors=new Object[]{
        ...
        new Connection.Month[]{new Connection.Month(11)},
        new Date[]{dateNow()},
        new java.util.Date[]{new java.util.Date()}, // datetime
        new Connection.Timespan[]{new Connection.Timespan()},
        new Connection.Minute[]{new Connection.Minute((int)timeNow().getTime()/60000)},
        new Connection.Second[]{new Connection.Second((int)timeNow().getTime()/1000)},
        new Time[]{timeNow()}};
      ...
      for(Object vector:vectors){
        for(int i=0;i<2;i++){
          ...
          Object[]result=(Object[]) Connection.k("{(-3!x;type x;x)}",arg); // returns a 3 element list of (stringified x; the type number of x; x)
      ...
      LOGGER.log(Level.INFO,"{0}",String.format(format,"","","","","").replace(' ', '-'));
      Object result= Connection.k("{x}",vectors);
      LOGGER.log(Level.INFO,"List Roundtrip match: {0}",Arrays.deepEquals(new Object[]{result},new Object[]{vectors}));

      Connection.Dict dict=new Connection.Dict(new String[]{"Andrew","James"},new int[]{24,35});
      result= Connection.k("{x}",dict);
      LOGGER.log(Level.INFO,"Dict Roundtrip match: {0}",(Arrays.deepEquals(new Object[]{dict.x},new Object[]{((Connection.Dict)result).x})
                        && Arrays.deepEquals(new Object[]{dict.y},new Object[]{((Connection.Dict)result).y})));

      Connection.Flip flip=new Connection.Flip(new Connection.Dict(new String[]{"time","sym","price","volume"},
                                         new Object[]{new Connection.Timespan[]{new Connection.Timespan(),new Connection.Timespan()},
                                                      new String[]{"ABC","DEF"},
                                                      new double[]{123.456,789.012},
                                                      new long[]{100,200}
                                             }));
      result= Connection.k("{x}",flip);
      LOGGER.log(Level.INFO,"Flip Roundtrip match: {0}",(Arrays.deepEquals(new Object[]{flip.x},new Object[]{((Connection.Flip)result).x})
                        && Arrays.deepEquals(flip.y,((Connection.Flip)result).y)));
    }catch(Exception e){

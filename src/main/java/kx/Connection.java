...
public class Connection
{
  ...
  */
  ...
  /**
   * Initializes a new {@link Connection} instance by acting as a server, blocking
   ...
   * 
   */
  public Connection(ServerSocket s, IAuthenticate a) throws IOException{
    io(s.accept());
    rBuff=new byte[99];
  ...
  /** 
   * Initializes a new {@link Connection} instance by acting as a server, and blocks while waiting
   * for a new client connection. A new instance should be created for each new client connection.
   * @param s {@link ServerSocket} to accept connections on using kdb+ IPC protocol.
   * @throws IOException an I/O error occurs.
   */
  public Connection(ServerSocket s) throws IOException{
    this(s,null);
  }
  /**
   * Initializes a new {@link Connection} instance and connects to KDB+ over TCP.
   ...
   */
  public Connection(String host, int port, String usernamepassword) throws KException,IOException{
    this(host,port,usernamepassword,false);
  }

  /**
   * Initializes a new {@link Connection} instance and connects to KDB+ over TCP with optional TLS support for encryption.
   * @param host Host of remote q process
   * @param port Port of remote q process
   * @param usernamepassword Username and password as "username:password" for remote authorization
   ...
   * @throws IOException if an I/O error occurs.
   */
  public Connection(String host, int port, String usernamepassword, boolean useTLS) throws KException,IOException{
  ...
  /**
   * Initializes a new {@link Connection} instance and connects to KDB+ over TCP, using {@code user.name} system property for username and password criteria.
   ...
   * @throws IOException if an I/O error occurs.
   */
  public Connection(String host, int port) throws KException,IOException{
    this(host,port,System.getProperty("user.name"));
  }
  /** Initializes a new {@link Connection} instance for the purposes of serialization only, no connection is instantiated
...
//object.getClass().isArray()   t(int[]) is .5 isarray is .1 lookup .05
  ...
  public interface MsgHandler{
     ...
     * @param Connection The c object that received the message
     ...
     */
    default void processMsg(Connection Connection, byte msgType, Object msg)throws IOException{
      switch(msgType){
        case 0:System.err.println("discarded unexpected incoming async msg!");break; // implicitly discard incoming async messages
        case 1:
          Connection.ke("unable to process sync requests");break; // signal to remote that we're unable to process these by default
